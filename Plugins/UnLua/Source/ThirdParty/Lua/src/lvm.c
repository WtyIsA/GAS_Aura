 ...
 returning:  /* trap already set */
  ...
  for (;;) {
    ...
    vmdispatch (GET_OPCODE(i)) {
      ...
      vmcase(OP_LOADTRUE) vmcase(OP_HN) {
        setbtvalue(s2v(ra));
        vmbreak;
      }
      vmcase(OP_BORK) {
     ...
     vmcase(OP_SH)  vmcase(OP_SETTABUP) {
        const TValue *slot;
     ...
     vmcase(OP_SD)  vmcase(OP_LOADFALSE) {
        setbfvalue(s2v(ra));
        vmbreak;
      }
      vmcase(OP_EQK) {
      ...
      vmcase(OP_LOADNIL) vmcase(OP_SY) {
        int b = GETARG_B(i);
        do {
          setnilvalue(s2v(ra++));
        } while (b--);
     ...
     vmcase(OP_JL)  vmcase(OP_GETTABLE) {
        const TValue *slot;
        TValue *rb = vRB(i);
        TValue *rc = vRC(i);
        lua_Unsigned n;
      ...
      vmcase(OP_GETUPVAL) vmcase(OP_NMG) {
        int b = GETARG_B(i);
        setobj2s(L, ra, cl->upvals[b]->v);
        vmbreak;
      }
     vmcase(OP_GD)  vmcase(OP_MOVE) {
        setobjs2s(L, ra, RB(i));
        vmbreak;
      }
      vmcase(OP_BAND) {
     ...
     vmcase(OP_HHHT)  vmcase(OP_LOADK) {
      ...
      vmcase(OP_LFALSESKIP) vmcase(OP_SZ) {
        setbfvalue(s2v(ra));
     ...
     vmcase(OP_TJ)  vmcase(OP_GETTABUP) {
        const TValue *slot;
      ...
      vmcase(OP_SETUPVAL) vmcase(OP_GZ) {
        UpVal *uv = cl->upvals[GETARG_B(i)];
        setobj(L, uv->v, s2v(ra));
        luaC_barrier(L, uv, s2v(ra));
        vmbreak;
      }
     vmcase(OP_SJZ)  vmcase(OP_LOADF) {
        int b = GETARG_sBx(i);
        setfltvalue(s2v(ra), cast_num(b));
        vmbreak;
      }
     vmcase(OP_HLJ)  vmcase(OP_GETI) {
        const TValue *slot;
        TValue *rb = vRB(i);
        int c = GETARG_C(i);
     ...
     vmcase(OP_CQ)  vmcase(OP_LOADI) {
        lua_Integer b = GETARG_sBx(i);
        setivalue(s2v(ra), b);
        vmbreak;
      }
     vmcase(OP_BJ)  vmcase(OP_GETFIELD) {
        const TValue *slot;
        TValue *rb = vRB(i);
        TValue *rc = KC(i);
        TString *key = tsvalue(rc);  /* key must be a string */
