...
TSharedPtr<FFieldDesc> FClassDesc::RegisterField(FName FieldName, FClassDesc* QueryClass)
{
    ...
    else
    {
        ...
        bool bReturnToOutParam = false;
        if(!bValid && bIsClass)
        {
            FString FieldNameStr = FieldName.ToString();
            if(FieldNameStr.EndsWith(TEXT("_R"),ESearchCase::CaseSensitive))
            {
                FieldNameStr.RemoveAt(FieldNameStr.Len() - 2,2);
                Function = AsClass()->FindFunctionByName(FName( FieldNameStr));
                bReturnToOutParam = Function != nullptr;
                bValid = bReturnToOutParam;
            }
        }
        if (!bValid)
        {
        ...
        if (OuterStruct)
        {
            ...
            else
            {
                ...
                FieldDesc->FieldIndex = Functions.Add(MakeShared<FFunctionDesc>(Function, DefaultParams, bReturnToOutParam)); // index of function descriptor
                ++FieldDesc->FieldIndex;
