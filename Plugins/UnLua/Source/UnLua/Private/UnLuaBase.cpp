...
namespace UnLua
{
    ...
    bool LoadChunk(lua_State *L, const char *Chunk, int32 ChunkSize, const char *ChunkName, const char *Mode, int32 Env)
    {
        ...
        if (Code != LUA_OK)
        {
            UE_LOG(LogUnLua, Warning, TEXT("Failed to call luaL_loadbufferx, error code: %d, in unluaBase %s"), Code, UTF8_TO_TCHAR(ChunkName));
    ...
    void DelegateUnBind(const FScriptDelegate *ScriptDelegate)
    {
        if(!ScriptDelegate)
            return;
        const auto Registry = UnLua::FLuaEnv::FindEnvChecked(GetState()).GetDelegateRegistry();
        Registry->ReleaseHandle(ScriptDelegate->GetUObject());
    }
    bool ReleaseLuaCallback(const UObject* Object, const FString& funName)
    {
        const auto Registry = UnLua::FLuaEnv::FindEnvChecked(GetState()).GetDelegateRegistry();
        Registry->ReleaseHandle(Object);
        return true;
    }

    bool ReleaseLuaCallback(const UObject* Object, const void* luaFun)
    {
        const auto Registry = UnLua::FLuaEnv::FindEnvChecked(GetState()).GetDelegateRegistry();
        //Registry->Remove()
        Registry->ReleaseHandle(Object, luaFun);
        return true;
    }

    void AddObjectReferencer(UObject* Object)
    {
        const auto Env = IUnLuaModule::Get().GetEnv();
        Env->GetObjectReferencer().Add(Object);
    }

    void RemoveObjectReferencer(UObject* Object)
    {
        const auto Env = IUnLuaModule::Get().GetEnv();
        Env->GetObjectReferencer().Remove(Object);
    }

    const FString& GetDonwloadResPath()
    {
        static  FString         _DownloadResFullpath;
        if(_DownloadResFullpath.Len() == 0)
        {
            FString ProjectPersistentDownloadDir = FPaths::ConvertRelativePathToFull(FPaths::ProjectPersistentDownloadDir());        
            _DownloadResFullpath = FPaths::Combine(ProjectPersistentDownloadDir, TEXT("Res/"));
        }
        return _DownloadResFullpath;
    }

    void SetLuaSearchPaths(const TArray<FString>& luaPaths)
    {
        FLuaEnv::SetLuaSearchPaths(luaPaths);
    }
