...
int FCfgManager::GetCfgType(const char* str)
{
	if(str == nullptr)
	{
		UE_LOG(GAME, Error,TEXT("[cfg]FCfgManager::GetCfgType str null"))
		return 0;
	}
	...
	if(pId)
		return *pId;
	UE_LOG(GAME, Error,TEXT("[cfg]FCfgManager::GetCfgType error str=%s "),*strType);
	return 0;
}
...
bool FCfgManager::RegisBean(FString& info)
{
	...
	if(doc.IsArray())
	{
		UE_LOG(GAME, Error, TEXT("[cfg]FCfgManager::RegisBean error"));
		return false;
	}
...
SCfgItemRow* FCfgManager::RegisConstTable(FString& info)
{
	...
	if (doc.IsArray())
	{
		UE_LOG(GAME, Error, TEXT("[cfg]FCfgManager::RegisConstTable error"));
		return nullptr;
	}
	SCfgStructInfo* pStructInfo = new SCfgStructInfo();
	pStructInfo->m_cfgName = FString(doc["name"].GetString());
...
Table* FCfgManager::RegisTable(FString& info)
{
	...
	if (doc.IsArray())
	{
		UE_LOG(GAME, Error, TEXT("[cfg]FCfgManager::RegisTable error"));
		return nullptr;
	}
	SCfgStructInfo* pStructInfo = new SCfgStructInfo();
	pStructInfo->m_cfgName = FString(doc["name"].GetString());
	...
	{
		UE_LOG(GAME, Error, TEXT("[cfg]FCfgManager::RegisTable error, field num is 0"))
		return nullptr;
	}
