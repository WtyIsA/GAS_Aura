...
int RegisBean(lua_State *L)
{
	...
	if(nParamNum != 1)
	{
		UE_LOG(GAME,Error,TEXT("[cfg]RegisBean error param  num=%d"),nParamNum);
	...
	if(Type != LUA_TSTRING)
	{
		UE_LOG(GAME,Error,TEXT("[cfg]RegisBean error param1(%d) not string"),Type);
		CLuaMessageTableOptInC::OnlyPrintLuaTrackback();
		return 0;
	}
...
int RegisConstTable(lua_State *L)
{
	...
	if(nParamNum != 1)
	{
		UE_LOG(GAME,Error,TEXT("RegisBean error param  num=%d"),nParamNum);
		CLuaMessageTableOptInC::OnlyPrintLuaTrackback();
		return 0;
	}
	int32 Type = lua_type(L, 1);
...
int RegisTable(lua_State *L)
{
	...
	if(nParamNum != 1)
	{
		UE_LOG(GAME,Error,TEXT("[cfg]RegisTable error param  num=%d"),nParamNum);
		CLuaMessageTableOptInC::OnlyPrintLuaTrackback();
		return 0;
	}
	int32 Type = lua_type(L, 1);
	if(Type != LUA_TSTRING)
	{
		UE_LOG(GAME,Error,TEXT("[cfg]RegisTable error param1(%d) not string"),Type);
		CLuaMessageTableOptInC::OnlyPrintLuaTrackback();
		return 0;
	}
	const char* pStr = lua_tostring(L, 1);
...
int GetLuaCfgTableLen(lua_State*L)
{
	...
	if(nParamNum < 1 )
	{
		UE_LOG(GAME,Error,TEXT("[cfg]LuaCfgGetLen error param  num=%d"),nParamNum);
		CLuaMessageTableOptInC::OnlyPrintLuaTrackback();
		return 0;
	}
	...
	if(nType != LUA_TTABLE)
	{
		UE_LOG(GAME,Error,TEXT("[cfg]GetLuaCfgTableLen  param(%d) should be table"),nType);
		CLuaMessageTableOptInC::OnlyPrintLuaTrackback();
		lua_pushinteger(L,0);
		return 1;
	}
...
int LuaCfgTable_Index(lua_State*L)
{
	...
	if(nParamNum != 2 )
	{
		UE_LOG(GAME,Error,TEXT("[cfg]LuaCfgTable_Index error param  num=%d"),nParamNum);
	...
	if(pCTab == nullptr)
	{
		UE_LOG(GAME,Error,TEXT("[cfg]LuaCfgTable_Index error table not exist"));
		CLuaMessageTableOptInC::OnlyPrintLuaTrackback(nullptr, L);
		return 0;
	}
...
int LuaCfgTable_NewIndex(lua_State*L)
{
	...
	if(nParamNum != 3)
	{
		UE_LOG(GAME,Warning,TEXT("[cfg]LuaCfgGet_NewIndex param error %d"), nParamNum);
		CLuaMessageTableOptInC::OnlyPrintLuaTrackback();
		return 0;
	}
	lua_rawset(L,-3);
...
int IterLuaTable(lua_State *L)
{
	...
	if(nParamNum != 2)
	{
		UE_LOG(GAME,Warning,TEXT("[cfg]IterLuaTable error params  num(%d) !=2"),nParamNum);
		CLuaMessageTableOptInC::OnlyPrintLuaTrackback();
		return 0;
	}
	const void* p = lua_topointer(L,1);
	...
	{
		UE_LOG(GAME,Warning,TEXT("[cfg]IterLuaTable error name not exist %p"),p);
		return  CLuaMessageTableOptInC::PrintLuaTrackbackAndPushNil(nullptr,L);
	}
	...
	{
		UE_LOG(GAME,Warning,TEXT("[cfg]IterLuaTable  from  key type error %d"), Type);
		return  CLuaMessageTableOptInC::PrintLuaTrackbackAndPushNil(nullptr,L);
	}	
	return 0;
}
...
int LuaCfgTable_pairs(lua_State* L)
{
	...
	if(nParamNum != 1)
	{
		UE_LOG(GAME,Warning,TEXT("[cfg]LuaCfgTable_pairs param error %d"), nParamNum);
		return  CLuaMessageTableOptInC::PrintLuaTrackbackAndPushNil(nullptr,L);
	}
	if(lua_type(L,1) == LUA_TTABLE)
	{
		...
		{
			UE_LOG(GAME,Warning,TEXT("[cfg]LuaCfgTable_pairs param table null"));
			return  CLuaMessageTableOptInC::PrintLuaTrackbackAndPushNil(nullptr,L);
		}
	}
	return 0;
...
int GetCfgRowItem_Index(lua_State *L)
{
	...
	if(nParamNum < 2)
	{
		UE_LOG(GAME,Warning,TEXT("[cfg]GetCfgItem_Index error, params num=%d"),nParamNum);
		CLuaMessageTableOptInC::OnlyPrintLuaTrackback();
		return 0;
	}
	...
	if(pRow == nullptr)
	{
		UE_LOG(GAME,Warning,TEXT("[cfg]GetCfgRowItem_Index ud is null"));
		return  CLuaMessageTableOptInC::PrintLuaTrackbackAndPushNil(nullptr,L);
	}
	int nluaType = lua_type(L, 2);
	if(nluaType != LUA_TSTRING)
	{
		UE_LOG(GAME,Warning,TEXT("[cfg]GetCfgRowItem_Index error, param2(%d) not string" ),nluaType);
		return  CLuaMessageTableOptInC::PrintLuaTrackbackAndPushNil(nullptr,L);
	}
	FCfgTable* pCfgTable = FCfgManager::Get()->GetCfgTable(pRow->m_beanId);
	if(pCfgTable == nullptr)
	{
		UE_LOG(GAME,Warning,TEXT("[cfg]GetCfgRowItem_Index error, beanId not exist %d"), pRow->m_beanId);
		return  CLuaMessageTableOptInC::PrintLuaTrackbackAndPushNil(nullptr,L);
	}
...
int GetCfgBeanItem_Index(lua_State *L)
{
	...
	if(nParamNum < 2)
	{
		UE_LOG(GAME,Warning,TEXT("[cfg]GetCfgBeanItem_Index error, params num=%d"),nParamNum);
		CLuaMessageTableOptInC::OnlyPrintLuaTrackback();
		return 0;
	}
	void* pUd =  lua_touserdata(L,1);
	...
	if(pRow == nullptr)
	{
		UE_LOG(GAME,Warning,TEXT("[cfg]GetCfgBeanItem_Index usedata null num=%d"),nParamNum);
		CLuaMessageTableOptInC::OnlyPrintLuaTrackback();
		return  0;
	}
	int nluaType = lua_type(L, 2);
	if(nluaType != LUA_TSTRING)
	{
		UE_LOG(GAME,Warning,TEXT("[cfg]GetCfgBeanItem_Index error, param2(%d) not string" ),nluaType);
		return  CLuaMessageTableOptInC::PrintLuaTrackbackAndPushNil(nullptr,L);
	}	
	FString strItemName( lua_tostring(L,2));
...
int GetListItem(lua_State *L)
{
	...
	if(nParamNum != 2)
	{
		UE_LOG(GAME,Warning,TEXT("[cfg]GetListItem error, params num=%d"),nParamNum);
		CLuaMessageTableOptInC::OnlyPrintLuaTrackback();
		return 0;
	}
	void* pUd =  lua_touserdata(L,1);
	...
	if(pRow == nullptr)
	{
		UE_LOG(GAME,Warning,TEXT("[cfg]GetListItem usedata is null"));
		CLuaMessageTableOptInC::OnlyPrintLuaTrackback();
		return  0;
	}
	int nluaType = lua_type(L, 2);
	...
	{
		UE_LOG(GAME,Warning,TEXT("[cfg]GetListItem error, param2(%d) not int"), nluaType);
		CLuaMessageTableOptInC::OnlyPrintLuaTrackback();
		return 0;
	}
...
int GetListSetMapSize(lua_State* L ,const char* metaName)
{
	...
	if(nParamNum != 1)
	{
		UE_LOG(GAME,Warning,TEXT("[cfg]GetListSetMapSize error, params num=%d"),nParamNum);
		CLuaMessageTableOptInC::OnlyPrintLuaTrackback();
		return 0;
	}
	...
	if(pRow == nullptr)
	{
		UE_LOG(GAME,Warning,TEXT("[cfg]GetListSetMapSize row error=%d"),nParamNum);
		CLuaMessageTableOptInC::OnlyPrintLuaTrackback();
		return  0;
	}
...
int CheckSetContain(lua_State *L)
{
	...
	if(nParamNum != 2)
	{
		UE_LOG(GAME,Warning,TEXT("[cfg]CheckSetContain error, params num=%d"),nParamNum);
		CLuaMessageTableOptInC::OnlyPrintLuaTrackback();
		return 0;
	}
	...
	if(pRow == nullptr)
	{
		UE_LOG(GAME,Warning,TEXT("[cfg]CheckSetContain usedata null"));
		CLuaMessageTableOptInC::OnlyPrintLuaTrackback();
		return  0;
	}
	int32 Type = lua_type(L, 2);
...
int GetMapItem(lua_State *L)
{
	...
	if(nParamNum != 2)
	{
		UE_LOG(GAME,Warning,TEXT("[cfg]GetMapItem error, params num=%d"),nParamNum);
		CLuaMessageTableOptInC::OnlyPrintLuaTrackback();
		return 0;
	}
	...
	if(pRow == nullptr)
	{
		UE_LOG(GAME,Warning,TEXT("[cfg]GetMapItem usedata null"));
		CLuaMessageTableOptInC::OnlyPrintLuaTrackback();
		return  0;
	}
	int nluaType = lua_type(L, 2);
...
int GetMapItemByIndex(lua_State *L)
{
	...
	if(nParamNum != 2)
	{
		UE_LOG(GAME,Warning,TEXT("[cfg]GetMapItemByIndex error, params num=%d"),nParamNum);
		CLuaMessageTableOptInC::OnlyPrintLuaTrackback();
		return 0;
	}
	//SCfgMapRow* pRow = (SCfgMapRow*)*(void**)luaL_checkudata(L,1,__CfgMapMetaName__);
	...
	if(pRow == nullptr)
	{
		UE_LOG(GAME,Warning,TEXT("[cfg]GetMapItemByIndex usedata null"));
		CLuaMessageTableOptInC::OnlyPrintLuaTrackback();
		return  0;
	}
	int nluaType = lua_type(L, 2);
	...
	{
		UE_LOG(GAME, Warning, TEXT("[cfg]GetMapItemByIndex param type(%d) error not number"), nluaType);
	}
...
int Cfg_Gc(lua_State *L, const char * metaName)
{
	...
	if(pRow == nullptr)
	{
		UE_LOG(GAME,Warning,TEXT("[cfg]Cfg_Gc errer,not SCfgItemRow"));
		return  0;
	}
