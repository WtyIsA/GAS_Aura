...
uint8* FCfgTable::Generate(uint8* pBuffer,uint32& nSize, int ValueType, uint32 bufferLen)
{
	...
	{
		UE_LOG(GAME, Error, TEXT("[cfg]FCfgTable GenerateList error, value can not be list,set,map"));
		return  nullptr;
	}
	else if(ValueType >= CFG_TYPE_BEAN)
	{
		...
		{
			UE_LOG(GAME, Error, TEXT("[cfg]FCfgTable GenerateBean in list error, bean not exist %d"), ValueType);
			return nullptr;
		}
		for(uint16 i = 0; i < count; i++)
		{
...
uint8* FCfgTable::Generate(uint8* pBuffer,uint32& nSize, int ValueType, int keyType, uint32 bufferLen)
{
	...
	for(uint16 i = 0; i < count; i++)
	{
		...
		{
			UE_LOG(GAME, Error, TEXT("[cfg]oc %s map key(%d) error"), *m_pInfo->m_cfgName, keyType);
			return  nullptr;
		}
		pDest += keyLen;
		...
		else if(ValueType >= CFG_TYPE_BEAN)
		{
			...
			if (otherInfo == nullptr)
			{
				UE_LOG(GAME, Error, TEXT("[cfg]FCfgTable %s map error, bean not exist %d"),*m_pInfo->m_cfgName, ValueType);
				return nullptr;
			}
			uint32 nTempSize = 0;
			uint8* pStr = Generate(pBuffer + nSize, nTempSize, otherInfo,bufferLen - nSize);
		...
		{
			UE_LOG(GAME, Error, TEXT("[cfg]oc %s map value(%d) error"), *m_pInfo->m_cfgName, ValueType);
			return  nullptr;	
		}
		pDest += itemLen;
	}
...
uint8* FCfgTable::Generate(uint8* pBuffer,uint32& nSize, SCfgStructInfo* pStructInfo, uint32 bufferLen)
{
	...
	for (int i = 0; i < pStructInfo->m_Items.Num(); i++)
	{
		...
		else
		{
			...
			if (otherInfo == nullptr)
			{
				UE_LOG(GAME, Error, TEXT("[cfg]FCfgTable GenerateBean error, bean not exist %d"), item.nType);
				return nullptr;
			}
			uint32 nTempSize = 0;
			uint8* pStr = Generate(pBuffer + nSize, nTempSize, otherInfo,bufferLen - nSize);
...
bool FCfgTable::GenerateRows(uint8* pBuffer, uint32 nLen)
{
	...
	{
		UE_LOG(GAME, Error, TEXT("[cfg]oc error,key type(%d) invalid in table( %s)"),m_pInfo->m_keyType,  *m_pInfo->m_cfgName);
		return false;
	}
	try
	{
		...
		while (nSize  < nLen)
		{
			...
			if (pOneRow == nullptr)
			{
				UE_LOG(GAME, Error, TEXT("[cfg]oc error,table :%s"), *m_pInfo->m_cfgName);
				return false;
			}
			//get key
			if (m_pInfo->m_keyType == CFG_TYPE_INT)
	...
	{
		UE_LOG(GAME, Error, TEXT("[cfg]FCfgTable load oc error %s"), *m_pInfo->m_cfgName)
		return false;
	}
	return true;
}
...
bool   FCfgTable::GetColumnByName(SCfgItemRow *pItem, FString& strName, lua_State*L)
{
	...
	{
		UE_LOG(GAME, Error, TEXT("[cfg]FCfgTable::GetColumnByName Column(%s) not exist in %s"), *strName, *m_pInfo->m_cfgName)
		return false;
	}
...
bool FCfgTable::GetBeanColumnByName(SCfgItemRow *Item, FString& strName, lua_State*L)
{
	...
	if(pBean == nullptr)
	{
		UE_LOG(GAME, Warning, TEXT("[cfg]GetBeanColumnByName error, id=%d"), Item->m_beanId)
		return false;
	}
	int* pId = pBean->m_nameToIndex.Find(strName);
	if(pId == nullptr || *pId < 0 || *pId >= pBean->m_Items.Num())
	{
		UE_LOG(GAME, Error, TEXT("[cfg]FCfgTable::GetBeanColumnByName Column(%s) not exist in %s"), *strName, *pBean->m_cfgName)
		return false;
	}
...
bool   FCfgTable::GetTValueFromBuffer(TValue* pValue, uint8* pBuffer, int ValueType)
{
	...
	case CFG_TYPE_SET:
	case CFG_TYPE_MAP:
		UE_LOG(GAME, Warning,TEXT("[cfg]GetTValueFromBuffer kay value error:%d"), ValueType);
		return false;
	default:
		{
			...
			if(pItemRom->m_luaUd == nullptr)
			{
				...
				if (Type == LUA_TTABLE)
				{
					...
					UE_LOG(GAME,Error , TEXT("cfg rom setmetabatle error %s: %d"), UTF8_TO_TCHAR(__CfgBeanMetaName__), Type)
				}
				pItemRom->m_luaUd = ud;
				lua_pop(L,1);
			}
	...
	case CFG_TYPE_LIST:
		{
			...
			if(pItemRom == nullptr)
			{
				UE_LOG(GAME, Error,TEXT("[cfg]PushSubItemToLua list null"))
				return false;
			}
	...
	case CFG_TYPE_SET:
		{
			...
			if(pItemRom == nullptr)
			{
				UE_LOG(GAME, Error,TEXT("[cfg]PushSubItemToLua set null"))
				return false;
			}
			int listCount = GetListSize(pItemRom);
	...
	case CFG_TYPE_MAP:
		{
			...
			if(pItemRom == nullptr)
			{
				UE_LOG(GAME, Error,TEXT("[cfg]PushSubItemToLua map null"))
				return false;
			}
			int listCount = GetListSize(pItemRom);
...
bool FCfgTable::PushItemRowToLua(SCfgItemRow *pItemRom, const char* metaName, lua_State*L)
{
	...
	if(pItemRom->m_luaUd == nullptr)
	{
		...
		if (Type == LUA_TTABLE)
		{
			...
			UE_LOG(GAME,Error , TEXT("cfg item setmetabatle error %s: %d"), UTF8_TO_TCHAR(metaName), Type)
		}
		pItemRom->m_luaUd = ud;
	}else
	{
...
bool FCfgTable::GetMapItemByIndex(SCfgMapRow* pItem, uint32 nIndex, lua_State*L)
{
	...
	{
		UE_LOG(GAME, Error, TEXT("[cfg]GetMapItemByIndex error, index %d out range[1,%d]"), nIndex, nMaxCount);
		return  false;
	}
	int itemLen = GetCfgTypeSize(pItem->m_beanId);
	int keyLen = GetCfgTypeSize(pItem->m_keyType);
