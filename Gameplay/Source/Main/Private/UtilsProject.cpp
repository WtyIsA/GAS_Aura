...
int32 FUtilsProject::UnMountSavePaks(bool bAutoDelete)
{
	FString path = GetPakSavePath();
	...
	IFileManager::Get().FindFiles(FoundPakFiles, *path, TEXT(".pak"));
	int nDeletePakNum = 0;
	for(auto name: FoundPakFiles)
	{
		FString pakFullPath = FPaths::Combine(path, name);
		PakFileMgr->Unmount(*pakFullPath);
		if(bAutoDelete)
		{
			if(!IFileManager::Get().Delete(*pakFullPath))
			{
				UE_LOG(GAME,Warning,TEXT("delete pak %s failed"),*pakFullPath);
			}
			else
				nDeletePakNum++;
		}
	}
	return nDeletePakNum;
}
...
int32 FUtilsProject::MountSavePaks()
{
	...
	//if(nPakNums > 0)
	{
		UE_LOG(GAME, Display, TEXT("======================== OpenLibrary Begin ========================="));
		...
		UE_LOG(GAME, Display, TEXT("======================== OpenLibrary End ========================="));

	}
	UE_LOG(GAME,Display,TEXT("TextLocalization ---------------"));
	...
	UE_LOG(GAME, Log, TEXT("======================== OpenPipelineFileCache End OpenPipelineFileCache=%d, ========================="), bRe);
