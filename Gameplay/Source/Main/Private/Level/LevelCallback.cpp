...
{
	if(IsInGPMLevels(levelName))
	{
		//UE_LOG(LogTemp,Display,TEXT("[Level]LogSceneStart %s"), *levelName);
		FSdkInterface::GetIns()->LogSceneStart(levelName);
	}
}
...
void FLevelCallback::PostLoadMapWithWorld(UWorld* World)
{
	...
	if(IsInGPMLevels(World->GetName()))
	{
		FSdkInterface::GetIns()->LogSceneLoaded();
		//UE_LOG(LogTemp,Display,TEXT("[Level]LogSceneLoaded %s"), *World->GetName());
	}
...
{	
	if(World)
	{
		if(IsInGPMLevels(World->GetName()))
		{
			//UE_LOG(LogTemp,Display,TEXT("[Level]LogSceneEnd %s"), World?*World->GetName():TEXT("none"));
			FSdkInterface::GetIns()->LogSceneEnd();
		}
