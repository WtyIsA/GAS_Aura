...
void FLuaGcTickable::Tick(float DeltaTime)
{
	...
	if (g->gcstate == GCSpause)
	{
		...
		GcOnceTime = 0;
		UE_LOG(GAME, Display, TEXT("[Lua]gc begin:mem=%dkb"), lua_gc(LuaState, LUA_GCCOUNT));
	}
	...
	do
	{
		...
		if (GLuaGcDebugLog)
		{
			UE_LOG(GAME, Display, TEXT("[Lua]gc step usetime=%.2fms"), gcStepTime * 1000);
		}
	...
	if (g->gcstate == GCSpause)
	{
		UE_LOG(GAME, Display, TEXT("[Lua]gc end:mem=%dkb,usedtime=%.2fms"), lua_gc(LuaState, LUA_GCCOUNT), GcOnceTime * 1000);
		GcOnceTime = 0.0;
	}
}
