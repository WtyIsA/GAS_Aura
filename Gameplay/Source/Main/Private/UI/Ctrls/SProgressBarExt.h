...
#include "SProgressBarExt.generated.h"
...
class SProgressBarExt : public SLeafWidget
{
public:
SLATE_BEGIN_ARGS(SProgressBarExt)
			: _Style(&FCoreStyle::Get().GetWidgetStyle<FProgressBarStyle>("ProgressBar"))
			  , _BarFillType(EFillMode::Type::LeftToRight)
			  , _Percent(TOptional<float>())
			  , _StartAngle(TOptional<float>())
		...
		SLATE_ARGUMENT(EFillMode::Type, BarFillType)

		/** Used to determine the fill position of the progress bar ranging 0..1 */
		SLATE_ATTRIBUTE(TOptional<float>, Percent)

		SLATE_ATTRIBUTE(TOptional<float>, StartAngle)
	...
	*/
	...
	/** See attribute StartAngle */
	void SetStartAngle(TAttribute<TOptional<float>> InStartAngle);

	/** See attribute Style */
	void SetStyle(const FProgressBarStyle* InStyle);

	/** See attribute BarFillType */
	void SetBarFillType(EFillMode::Type InBarFillType);
...
private:
	...
	TAttribute<TOptional<float>> StartAngle;

	EFillMode::Type BarFillType;
