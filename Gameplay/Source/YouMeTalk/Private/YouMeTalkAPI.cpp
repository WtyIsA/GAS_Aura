...
void UYouMeTalkAPI::SetUserVolume(const FString& userId, const int32& uiVolume)
{
	IYouMeVoiceEngine::getInstance()->setUserVolume(TCHAR_TO_ANSI(*userId), uiVolume);
}
...
EYouMeErrorCode UYouMeTalkAPI::SetFarendVoiceLevelCallback(int32 maxLevel, int32 maxMixedLevel)
{
	YouMeErrorCode errorcode = IYouMeVoiceEngine::getInstance()->setFarendVoiceLevelCallback(maxLevel, maxMixedLevel);
	return YouMeTalkDefineConvert::YouMeErrorCodeConvert(errorcode);
}
