...
public:
	...
	*/
    ...
    /**
    *  功能描述:设置远端单个用户音量增益(0-300)，100为正常值，0 为无声, 200以内相对安全，超过200可能有明显的破音
    *  退出频道时自动清除设置。
    *
    *  @return 无
    */
    YouMeErrorCode setUserVolume(const char* userID, const unsigned int &uiVolume)
#ifdef YOUME_MAC
	{
		YouMeErrorCode errorcode = (YouMeErrorCode_t)YOUME_SUCCESS;
		return errorcode;
...
//                                 bool bLittleEndian, bool bInterleaved, bool bForSpeaker);
	...
	*/
	...
	/**
	* 功能描述：给客户用于检测网络质量。检测结束后会收到onDetectNetWorkCompleteCb回调返回结果。
	*  @param nDetectTime:检测时长，单位为秒，默认3秒
	*  @param nOverTime:终止时常，单位为秒，默认4秒
	*  @param nInterval:发包间隔，单位毫秒，默认20毫秒
	*  @param nPackSize:包大小，单位字节，默认300字节
	*  @return YOUME_SUCCESS - 成功
    *       其他 - 具体错误码
	*/
	YouMeErrorCode detectNetworkQuality(int nDetectTime = 3, int nOverTime = 4, int nInterval = 20, int nPackSize = 300)
#ifdef YOUME_MAC
	{
		YouMeErrorCode errorcode = (YouMeErrorCode_t)YOUME_SUCCESS;
		return errorcode;
	...
	/**
	*  功能描述:detectNetworkQuality的回调消息
	*  @param pCallback: detectNetworkQuality的回调， 需要继承IYouMeDetectNetworkCallback并实现其中的回调函数
	*
	*  @return None
	*/
	void setDetectNetworkCallback(IYouMeDetectNetworkCallback* pCallback)
#ifdef YOUME_MAC
	{
	}
#else
	...
	/**
	* 功能描述：终止检测网络质量。终止后将不会收到onDetectNetWorkCompleteCb回调。
	*  @return YOUME_SUCCESS - 成功
	*       其他 - 具体错误码
	*/
