...
typedef enum YouMeEvent {
    ...
    YOUME_EVENT_IDENTITY_MODIFY             = 77,   ///角色改变, errcode为身份标识,YouMeUserRole
    
    YOUME_EVENT_REPORT_AUDIO                = 78,   ///<保存本地音频缓存到文件，用于上报
    YOUME_EVENT_SHOW_RECORD_ALERT           = 79,    ///<提示录音权限
...
typedef enum YouMeErrorCode {
    ...
    YOUME_ERROR_CREATE_FILE_FAIL             = -114,    ///创建文件失败
    YOUME_ERROR_FUNCTION_NOT_OPEN            = -115,    ///<功能未打开，可能某些必要的开放功能接口未调用
    ...
    YOUME_ERROR_RECORD_ALERT_REFUSED         = -206, ///< 录音权限提示，被拒绝了
	// 抢麦错误
	...
	// 网络质量检测相关错误
	YOUME_ERROR_FREQUENCY_TOO_HIGH = -701, ///< 检测频率过快
